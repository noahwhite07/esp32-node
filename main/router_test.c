...
{   
    ...
    sprintf(full_url, "http://%s/capture", server_url);
...
void on_image_downloaded(uint8_t* img_buff, size_t img_size ){
    ESP_LOGI(TAG, "Image downloaded sucessfully");

    // Save a reference to the image and record its size
    //uint8_t * image_buff = get_image_buffer();
    //size_t fetched_image_size = get_image_size();
...
void app_main(void) {
    ...
    download_img_params->camera = cameras[0];
